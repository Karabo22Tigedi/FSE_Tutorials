...
def add_transaction(
    ...
    if transaction_type not in TRANSACTION_TYPES:
        raise ValueError(f"Invalid transaction type: {transaction_type} is not valid. Must be income or expense.")
    if amount < 0:
        raise ValueError(f"Amount: {amount} is invalid. Do the right thing." )
...
def calculate_balance(transactions: List[dict]) -> Decimal:
    ...
    for i in range(len(transactions)):
        list_item = transactions[i]
        if list_item["type"] == "income":
            balance += Decimal(list_item["amount"])
        else:
            balance -= Decimal(list_item["amount"])
...
def check_budget(balance: Decimal, budget_limit: Decimal) -> Tuple[bool, str]:
    ...
    if balance <= budget_limit:
        ...
        return (False, f"Budget exceeded! Overspent by {format_currency(overspent)}")
